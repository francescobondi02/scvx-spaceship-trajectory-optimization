...
def _ex11_vis(sim_context: SimContext) -> Report:
    ...
    with gif_viz.data_file("Animation", MIME_MP4) as fn:
        create_animation(
            file_path=fn, sim_context=sim_context, figsize=(16, 16), dt=50, dpi=30, plot_limits=[[-12, 27], [-12, 12]]
        )
...
def get_exercise11():
    ...
    # configs = ["config_scenario4.yaml", "config_planet2.yaml", "config_satellites.yaml"]
    # configs = ["config_planet.yaml", "config_planet2.yaml", "config_planet3.yaml"]  # "config_planet.yaml"]
    # configs = ["config_planet4.yaml"]
    # configs = ["config_planet.yaml", "config_planet2.yaml", "config_planet3.yaml"]  # FUNZIONANO 900
    # configs = ["config_satellites.yaml"] #"config_satellites_diff.yaml"]
    # configs = ["config_satellites.yaml"]
    configs = [
        "config_satellites_diff.yaml",
        "config_satellites2.yaml",
        "config_satellites3.yaml",
        "config_satellites4.yaml",
    ]
